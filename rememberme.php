...
{
    $yhteys = db_connect();
    $query = "INSERT INTO remember_me_tokens(user_id, selector, hashed_validator, expiry) VALUES(?, ?, ?, ?)";
...
function find_rememberme_token(string $selector)
{
    ...
    $yhteys = db_connect();
    $query = "SELECT id, selector, hashed_validator, user_id, expiry FROM remember_me_tokens
...
function find_user_by_token(string $token)
{
    ...
    $users_id = $email = null;
    $yhteys = db_connect();
    $query = "SELECT users.id, email FROM users INNER JOIN remember_me_tokens ON user_id = users.id
          WHERE selector = ? AND expiry > now() LIMIT 1";
    $stmt = $yhteys->prepare($query);
...
function rememberme(int $user_id, int $day = 30)
{
    ...
    if (insert_rememberme_token($user_id, $selector, $hash_validator, $expiry)) {
        ...
        /* Huom. tätä tarvitaan vain tokenin poistoon remember_me_tokens-taulusta */
